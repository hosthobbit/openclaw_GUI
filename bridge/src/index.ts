...
import { createOpenClawClient, getOpenClawStatus } from './openclawClient';
...
const apiLimiterStore = new Map<RateKey, RateBucket>();
const wsLimiterStore = new Map<RateKey, RateBucket>();
const apiKeyLimiterStore = new Map<RateKey, RateBucket>();
const wsKeyLimiterStore = new Map<RateKey, RateBucket>();
...
function getIp(req: express.Request | http.IncomingMessage): string {
  const headers = (req as any).headers as http.IncomingHttpHeaders | undefined;
...
app.use((req, res, next) => {
  ...
  return cors({ origin, methods: ['GET', 'OPTIONS'] })(req, res, next);
});
...
function apiRateLimit(
  ...
  const ip = getIp(req);
  const ipKey = `${ip}:api`;
  const ipAllowed = checkRateLimit(apiLimiterStore, ipKey, 60_000, RATE_LIMIT_API_PER_MIN_IP);
  ...
  if (hasValidKey) {
    ...
    keyAllowed = checkRateLimit(
      apiKeyLimiterStore,
      keyBucket,
      60_000,
      RATE_LIMIT_API_PER_MIN_KEY
    );
  }
...
wss.on('connection', (socket: TrackedSocket, req) => {
  ...
  const withinIpLimit = checkRateLimit(
    ...
    rateKeyIp,
    60_000,
    RATE_LIMIT_WS_PER_MIN_IP
  );

  let withinKeyLimit = true;
  if (allowed) {
    const rateKeyKey = `${apiKey}:ws`;
    withinKeyLimit = checkRateLimit(
      wsKeyLimiterStore,
      rateKeyKey,
      60_000,
      RATE_LIMIT_WS_PER_MIN_KEY
    );
  }
...
const heartbeatInterval = setInterval(() => {
  for (const clientSocket of wss.clients) {
    const s = clientSocket as TrackedSocket;
    if (s.isAlive === false) {
      s.terminate();
      continue;
    }
